...
class GaussianDiscriminantAnalysis:
    ...
    def pred(self, X_test : np.ndarray) -> np.ndarray:
        ...
        for i in range(len(self.classes)):
            predicted[i] = (np.log(self.probabilities[i]) - (0.5)*np.log(np.linalg.norm(self.covariances[i])) - (0.5)*(X_test - self.means[i])@np.linalg.solve(self.covariances[i], (X_test - self.means[i]).T)).diagonal()
